...
function importJsonFile() {
    ...
    reader.onload = function (e) {
        try {
            ...
            if (importedData && importedData.chatevents) {
                events = importedData.chatevents; // Importierte Events in das events-Array laden
...
function saveToFile() {
    ...
    const data = JSON.stringify({chatevents: events}, null, 4);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  //  jsonOutputDiv.insertBefore(formatButton, textarea);
   ...
   // jsonOutputDiv.insertBefore(copyButton, textarea);
    ...
    editor.setValue(JSON.stringify({ chatevents: events }, null, 2));
    ...
    editor.on('change', () => {
        try {
            ...
            if (updatedContent && Array.isArray(updatedContent.chatevents)) {
                events = updatedContent.chatevents; // Aktualisiere die bestehende events-Variable
...
function updateJsonOutput() {
    ...
    if (window.editor) {
        window.editor.setValue(JSON.stringify({chatevents: events}, null, 2));
        renderEventList();
    } else {
