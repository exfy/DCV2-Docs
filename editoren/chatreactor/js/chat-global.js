...
function importJsonFile() {
    ...
    reader.onload = function (e) {
        try {
            ...
            if (importedData && importedData.chatreactorevents) {
                events = importedData.chatreactorevents; // Importierte Events in das events-Array laden
...
function saveToFile() {
    ...
    const data = JSON.stringify({chatreactorevents: events}, null, 4);
    ...
    a.href = url;
    a.download = 'ChatReactorEvents.json';  // Name der exportierten Datei
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  //  jsonOutputDiv.insertBefore(formatButton, textarea);
   ...
   // jsonOutputDiv.insertBefore(copyButton, textarea);
    ...
    editor.setValue(JSON.stringify({ chatreactorevents: events }, null, 2));
    ...
    editor.on('change', () => {
        try {
            ...
            if (updatedContent && Array.isArray(updatedContent.chatreactorevents)) {
                events = updatedContent.chatreactorevents; // Aktualisiere die bestehende events-Variable
...
function updateJsonOutput() {
    ...
    if (window.editor) {
        window.editor.setValue(JSON.stringify({chatreactorevents: events}, null, 2));
        renderEventList();
    } else {
