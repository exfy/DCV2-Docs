function showPasteEventPopup() {
    ...
    document.getElementById('popup-ok').onclick = function () {
        ...
        try {
            ...
            if (newEvent.hasOwnProperty('permission')) {
                events.push(newEvent);
                renderEventList();
                updateJsonOutput();
                closePopup();
            } else {
                alert('Ungültiges JSON: ' + 'Angegebenes Event ist wohl kein DynButton Event');
            }
        } catch (error) {
...
function editEvent(eventIndex) {
    ...
    const popupContent = `
        <div class="popup-content">
            ...
            <input type="text" id="popup-displayname" oninput="updatePermission()" value="${event.displayname}">
            ...
            <label for="popup-time">Position der Lampe:</label>
                 <label for="popup-permission">Permission:</label>
                <input type="text" id="popup-permission" value="${event.permission}" oninput="updatePermission()" placeholder="botctl.dyn.">


            <label for="popup-dynButtonType">Button Type</label>
            <select id="popup-dynButtonType" value="${event.dynButtonType}" required>
            
                <option ${event.dynButtonType === 'primary' ? 'selected' : ''}>primary</option>
                <option ${event.dynButtonType === 'secondary' ? 'selected' : ''}>secondary</option>
                <option ${event.dynButtonType === 'success' ? 'selected' : ''}>success</option>
                <option ${event.dynButtonType === 'danger' ? 'selected' : ''}>danger</option>
            </select>
                <label for="popup-buttonPreview">Vorschau:</label>
                <button id="popup-buttonPreview" class="discord-button ${event.dynButtonType}" disabled>${event.displayname}</button>
    ...
    document.getElementById('popup-save').onclick = function () {
        ...
        event.permission = document.getElementById('popup-permission').value;
        event.dynButtonType = document.getElementById('popup-dynButtonType').value;
        event.systemname = event.displayname.toLowerCase().replace(/[^a-z00-9]/g, '');
...
function initEventEditor() {
    document.getElementById('eventForm').addEventListener('submit', function (event) {
        ...
        const dynButtonType = document.getElementById('dynButtonType').value;
...
function renderEventList() {
    ...
    events.forEach((event, index) => {
        ...
        eventDiv.innerHTML = `<strong>Button Vorschau:</strong> (<button id="buttonPreview" class="discord-button ${event.dynButtonType}" disabled>${event.displayname}</button>) <br> ${event.comment} <br> Permission: ${event.permission} <br> <br> `;
