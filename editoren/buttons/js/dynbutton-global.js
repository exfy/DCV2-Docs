...
function importJsonFile() {
    ...
    reader.onload = function (e) {
        try {
            ...
            if (importedData && importedData.dynamicbuttonevents) {
                events = importedData.dynamicbuttonevents; // Importierte Events in das events-Array laden
...
function saveToFile() {
    ...
    const data = JSON.stringify({dynamicbuttonevents: events}, null, 4);
    ...
    a.href = url;
    a.download = 'DynamicButtonEvent.json';  // Name der exportierten Datei
...
function updateButtonPreview() {
    const buttonType = document.getElementById('dynButtonType').value;
    ...
    // Update den Button Text sofort beim Schreiben
    buttonPreview.textContent = displayNameInput || 'button'; // Fallback Text

    // Update die Button Klasse
    buttonPreview.className = `discord-button ${buttonType}`;
}

// Event Listener für den Button Typ
document.getElementById('dynButtonType').addEventListener('change', updateButtonPreview);

function updatePermission() {
    const permissionInput = document.getElementById('permission');
    const userInput = permissionInput.value.replace(/^botctl\.dyn\./, ''); // Entferne den Prefix, falls vorhanden
    permissionInput.value = `botctl.dyn.${userInput}`;
}