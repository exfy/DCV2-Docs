function showPasteEventPopup() {
    ...
    document.getElementById('popup-ok').onclick = function () {
        ...
        try {
            ...
            if (newEvent.hasOwnProperty('searchText')) {
                events.push(newEvent);
                renderEventList();
                updateJsonOutput();
                closePopup();
            } else {
                alert('Ungültiges JSON: ' + 'Angegebenes Event ist wohl kein ChatReactor Event');
            }
        } catch (error) {
...
function editEvent(eventIndex) {
    ...
    const popupContent = `
        <div class="popup-content">
            ...
            <label for="popup-textIsWithColorCodes">Chat mit Farb-Codes:</label>
            <select id="popup-textIsWithColorCodes" required>
                <option value="false" ${event.textIsWithColorCodes === 'false' ? 'selected' : ''}>Nein</option>
                <option value="true" ${event.textIsWithColorCodes === 'true' ? 'selected' : ''}>Ja</option>
            </select>

            <label for="popup-textIsCaseSensitive">Groß und Kleinschreibung beachten:</label>
            <select id="popup-textIsCaseSensitive" required>
                <option value="false" ${event.textIsCaseSensitive === 'false' ? 'selected' : ''}>Nein</option>
                <option value="true" ${event.textIsCaseSensitive === 'true' ? 'selected' : ''}>Ja</option>
            </select>

            <label for="popup-chatReactorMode">ChatReactor Modus:</label>
            <select id="popup-chatReactorMode" required>
                <option value="STARTWITH" ${event.chatReactorMode === 'STARTWITH' ? 'selected' : ''}>Beginnt mit (STARTWITH)</option>
                <option value="CONTAINS" ${event.chatReactorMode === 'CONTAINS' ? 'selected' : ''}>Enthält (CONTAINS)</option>
                <option value="NOTCONTAINS" ${event.chatReactorMode === 'NOTCONTAINS' ? 'selected' : ''}>Enthält nicht (NOTCONTAINS)</option>
                <option value="ENDSWITH" ${event.chatReactorMode === 'ENDSWITH' ? 'selected' : ''}>Endet mit (ENDSWITH)</option>
                <option value="EQUALS" ${event.chatReactorMode === 'EQUALS' ? 'selected' : ''}>Ist gleich (EQUALS)</option>
                <option value="NOTEQUALS" ${event.chatReactorMode === 'NOTEQUALS' ? 'selected' : ''}>Ist nicht gleich (NOTEQUALS)</option>
                <option value="EMPTY" ${event.chatReactorMode === 'EMPTY' ? 'selected' : ''}>Ist leer (EMPTY)</option>
            </select>

            <label for="popup-regex">Suche Text:</label>
            <input type="text" id="popup-regex" value="${event.searchText}">
    ...
    document.getElementById('popup-save').onclick = function () {
        ...
        event.textIsWithColorCodes = document.getElementById('popup-textIsWithColorCodes').value === 'true';
        event.textIsCaseSensitive = document.getElementById('popup-textIsCaseSensitive').value === 'true';
        event.chatReactorMode = document.getElementById('popup-chatReactorMode').value;
        event.searchText = document.getElementById('popup-regex').value;
...
function initEventEditor() {
    document.getElementById('eventForm').addEventListener('submit', function (event) {
        ...
        const textIsWithColorCodes = document.getElementById('textIsWithColorCodes').value;
        const textIsCaseSensitive = document.getElementById('textIsCaseSensitive').value;
        const chatReactorMode = document.getElementById('chatReactorMode').value;
        ...
        const newEvent = {
            ...
            conditions: [],
            comment: comment || '',
            textIsWithColorCodes: textIsWithColorCodes === 'true',
            textIsCaseSensitive: textIsCaseSensitive === 'true',
            chatReactorMode: chatReactorMode,
...
function renderEventList() {
    ...
    events.forEach((event, index) => {
        ...
        // Helper function für boolean display
        const getBooleanIcon = (value) => value ? '✅' : '❌';
        const getBooleanText = (value) => value ? 'Ja' : 'Nein';
        
        eventDiv.innerHTML = `
            <div style="border: 1px solid #4e567cff; background-color: #32324dff; padding: 10px; margin: 5px 0; border-radius: 5px;">
                <div style="font-size: 16px; font-weight: bold; margin-bottom: 10px;">${event.displayname}</div>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px;">
                    <div><strong>Suchtext:</strong> <code>${event.searchText || 'Nicht gesetzt'}</code></div>
                    <div><strong>Modus:</strong> <span style="background: #1b7042ff; padding: 2px 6px; border-radius: 3px;">${event.chatReactorMode}</span></div>
                </div>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px;">
                    <div><strong>Farb-Codes:</strong> ${getBooleanIcon(event.textIsWithColorCodes)} ${getBooleanText(event.textIsWithColorCodes)}</div>
                    <div><strong>Groß-/Kleinschreibung:</strong> ${getBooleanIcon(event.textIsCaseSensitive)} ${getBooleanText(event.textIsCaseSensitive)}</div>
                </div>
                
                ${event.comment ? `<div style="font-style: italic; color: #666; margin-top: 5px;"><strong>Kommentar:</strong> ${event.comment}</div>` : ''}
            </div><br><br>
        `;
