...
document.addEventListener('DOMContentLoaded', () => {
    ...
    editor.setValue(JSON.stringify({ timeevents: events }, null, 2));
    ...
    editor.on('change', () => {
        try {
            ...
            if (updatedContent && Array.isArray(updatedContent.timeevents)) {
                events = updatedContent.timeevents; // Aktualisiere die bestehende events-Variable
...
function updateJsonOutput() {
    ...
    if (window.editor) {
        window.editor.setValue(JSON.stringify({timeevents: events}, null, 2));
        renderEventList();
    } else {
