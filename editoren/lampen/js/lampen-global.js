...
document.addEventListener('DOMContentLoaded', () => {
 ...
 //   jsonOutputDiv.insertBefore(copyButton, textarea);
    ...
    editor.setValue(JSON.stringify({ lampenevents: events }, null, 2));
    ...
    editor.on('change', () => {
        try {
            ...
            if (updatedContent && Array.isArray(updatedContent.lampenevents)) {
                events = updatedContent.lampenevents; // Aktualisiere die bestehende events-Variable
...
function updateJsonOutput() {
    ...
    if (window.editor) {
        window.editor.setValue(JSON.stringify({lampenevents: events}, null, 2));
        renderEventList();
    } else {
